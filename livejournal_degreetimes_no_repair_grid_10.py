...
pr = []
fr = open('pr_heatmap_no_repair_grid_10_bin_0-3.txt')
for line in fr:
    x = []
    col = line.strip('\n').strip(' ').split(' ')
...
for i in np.arange(len(pr)):
    for j in np.arange(len(pr[i])):
        if pr[i][j] != 0:
            x.append(pow(gap_i[i],-0.9586)*pow(gap_j[j],0.2822))
            y.append(pr[i][j])

ax21 = plt.subplot(234)
ax21.clear()
...
ax21.set_ylabel('$\Lambda$',color='k',fontsize=textsize,labelpad=15,rotation=0,alpha=1)
ax21.text(-0.27,1.10,'d',color='k',fontsize=textsize,fontweight='bold',rotation=0,alpha=1,transform=ax21.transAxes)
ax21.text(0.65, 0.15,'$\\alpha=-0.96(3)$',fontsize=ticksize,color='k',fontweight='normal',rotation=0,alpha=1,transform=ax21.transAxes)
ax21.text(0.65, 0.05,'$\\beta=\ \ \ \ 0.28(3)$',fontsize=ticksize,color='k',fontweight='normal',rotation=0,alpha=1,transform=ax21.transAxes)
ax21.text(0.05, 0.90,'$R^2=0.90$',fontsize=ticksize,color='k',fontweight='normal',rotation=0,alpha=1,transform=ax21.transAxes)
